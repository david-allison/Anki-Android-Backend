...
`/tools/` Development environment scripts (diagnostics, CI machine setup). The
backend interface and translations are generated by `rslib-bridge`'s build
script (`proto.rs`, `fluent.rs`)
